...
export class PlanRestrictionService {
    ...
    async getUserPlanInfo(user: any): Promise<UserPlanInfo> {
        let dbPlan = null;
        if (user?.planId) {
            dbPlan = await this.planRepo.findOneBy({ id: user.planId });
        }
        
        const planName = user?.plan || "Free";
        if (!dbPlan && planName) {
            dbPlan = await this.planRepo.createQueryBuilder("p")
        ...
        if (dbPlan) {
            return {
                ...
                allowOnlineRegistration: (dbPlan as any).allowOnlineRegistration ?? true,
                allowPayment: (dbPlan as any).allowPayment ?? true,
                reportsLevel: (dbPlan as any).reportsLevel || "Advanced",
                allowCustomBranding: (dbPlan as any).allowCustomBranding ?? false
            };
        }
