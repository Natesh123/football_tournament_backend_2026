...
export class PlanService {
    ...
    /** Bulk update which plans are shown on the public landing page. */
    async updateLandingVisibility(selectedPlanIds: number[]) {
        const ids = (selectedPlanIds || []).map(id => Number(id)).filter(id => !isNaN(id));
        
        await this.planRepository.createQueryBuilder()
            .update(Plan)
            .set({ landingVisible: false })
            .execute();

        if (ids.length > 0) {
            await this.planRepository.createQueryBuilder()
                .update(Plan)
                .set({ landingVisible: true })
                .where("id IN (:...ids)", { ids })
                .execute();
        }
