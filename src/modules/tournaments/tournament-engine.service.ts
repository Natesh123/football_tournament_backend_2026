...
export class TournamentEngineService {
    ...
    async generateStructure(tournamentId: string, scheduleConfig?: any) {
        ...
        // Auto-detect schedule configuration if not explicitly passed
        const autoConfig = (scheduleConfig && scheduleConfig.startDate)
            ? scheduleConfig
            : (tournament.settings?.schedule?.startDate ? tournament.settings.schedule : null);

        // Matches are played at the tournament venue — validate it exists before
        // touching the existing structure when explicit auto-scheduling is requested
        ...
        if (autoConfig && autoConfig.startDate) {
            const venueNames = await this.getVenueAssignmentNames(tournamentId);
            if (venueNames.length > 0) {
                await this.applyAutoSchedule(tournamentId, autoConfig);
            }
        }
    ...
    async getStructure(tournamentId: string) {
        ...
        const matches = await this.matchRepo.find({
            ...
            relations: ["homeTeam", "awayTeam", "group", "stage", "matchSources"],
            order: {
                stage: { stage_order: "ASC" },
                round: "ASC",
                startTime: "ASC",
