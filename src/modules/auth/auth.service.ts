...
export async function registerUser(email: string, password: string, user_name: string, phone_number: string, plan?: string, planId?: number) {
    ...
    const selectedPlanId = planId ? Number(planId) : undefined;
    ...
    if (pendingUser) {
        ...
        if (selectedPlanId) pendingUser.planId = selectedPlanId;
        pendingUser.otp = otp;
...
export async function verifyOtp(email: string, otp: string) {
    ...
    const selectedPlanId = pendingUser.planId || 1;
    
    // Check if chosen plan is Free or Paid
    const { Plan } = require("../plans/plan.entity");
    const planRepo = AppDataSource.getRepository(Plan);
    let isFreePlan = selectedPlan.toLowerCase() === "free";

    if (selectedPlanId) {
        const foundPlan = await planRepo.findOne({ where: { id: selectedPlanId } });
        if (foundPlan) {
            isFreePlan = Number(foundPlan.monthlyPrice) === 0 || foundPlan.code.toLowerCase() === "free" || foundPlan.name.toLowerCase().includes("free");
        }
    }

    // Free plan users start Active (state = 1); non-free plan organizers start Inactive (state = 0) until Admin activates them.
    const initialStatus = isFreePlan ? 1 : 0;
    ...
    if (initialStatus === 0) {
        return {
            message: "Your OTP is verified! Since you chose a paid plan, your account is created as inactive pending Admin activation.",
            pendingApproval: true
        };
    }

    return {
        message: "Your account has been created and activated successfully! You can now log in.",
...
export async function loginUser(email: string, password: string) {
    ...
    // Only Active accounts (state === 1) may log in. Accounts with state !== 1 (e.g. paid plan registrations)
    // require manual Admin activation.
    if (user.state !== 1) {
        const adminUser = await userRepo.findOne({ where: [{ roleId: 1 }] });

        const adminDetails = {
            name: adminUser?.user_name || "ATB Sports Admin",
            email: adminUser?.email || process.env.ADMIN_CONTACT_EMAIL || "admin@atbsports.com",
            phone: adminUser?.phone_number || process.env.ADMIN_CONTACT_PHONE || "+91 98765 43210",
            bankName: process.env.ADMIN_BANK_NAME || "State Bank of India",
            accountNumber: process.env.ADMIN_BANK_ACCOUNT || "98765432101234",
            ifscCode: process.env.ADMIN_BANK_IFSC || "SBIN0001234",
            accountHolder: process.env.ADMIN_BANK_HOLDER || "ATB Sports Tournament Admin"
        };

        const inactiveErr: any = new Error("The administrator has not granted access to your account yet.");
        inactiveErr.inactiveAccount = true;
        inactiveErr.adminDetails = adminDetails;
        inactiveErr.userPlan = user.plan || "Paid Plan";
        throw inactiveErr;
    }
