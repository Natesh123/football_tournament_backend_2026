...
export async function register(req: Request, res: Response) {
    try {
        const { email, password, user_name, phone_number, plan, planId } = req.body;
        const result = await registerUser(email, password, user_name, phone_number, plan, planId);
        res.json(result);
    } catch (err: any) {
...
export async function login(req: Request, res: Response) {
    try {
        ...
        if (err.inactiveAccount) {
            return res.status(403).json({
                error: err.message,
                inactive: true,
                adminDetails: err.adminDetails,
                userPlan: err.userPlan
            });
        }
